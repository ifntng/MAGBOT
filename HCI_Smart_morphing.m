...
for step=1:10000
    ...
    P_cut=P_ini(80:780,660:1370,:);
    P_rgb=rgb2gray(P_cut);
    P_TW = im2bw(P_rgb,0.1);
    P_cut(:,:,1)=0;
    se = strel('disk',3);
    ...
    centers(:,1)=round((centers(:,1)));
    centers(:,2)=round((centers(:,2)));
    centersxy=[];
    ...
    centers(centers(:,1)<=15|centers(:,1)>=345)=0;
    centers(centers(:,2)<=15|centers(:,2)>=345)=0;
    centers(centers(:,1)==0|centers(:,2)==0,:)=[];
    flushdata(vid);
